        ...
        didSet {
            ...
            responseQuestion { [weak self] updatedCategories in
                DispatchQueue.main.async {
                    self?.enabledCategories = updatedCategories
                    print("api 요청 후 카테고리 개수: \(self?.enabledCategories.count)")
...
extension NotEnteredCheckListViewController : UITableViewDelegate, UITableViewDataSource, UIScrollViewDelegate {
    ...
    func numberOfSections(in tableView: UITableView) -> Int {
        return enabledCategories.count + 1
    }
        ...
        if isEditMode {
            if indexPath.row == 0 {
                ...
                case 0:
                    ...
                    cell.configure(with: questionDto)
